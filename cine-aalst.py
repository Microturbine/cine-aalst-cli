...
def print_movie(movie, schedules, cinemas, screens):
    ...
    # Check if description exists before removing HTML tags
    ...
    print(f"  - \033[1mPoster:\033[0m \x1B]8;;https://cine-aalst.be{movie['poster']}\x1B\\Link\x1B]8;;\x1B\\")
    ...
    for schedule in schedules:
        ...
        mask_emoji = "😷" if screen_details and screen_details['config']['social_distancing_enabled'] else ""
        ...
        print(f"    - {start_time.strftime('%A, %d %B %H:%M')}{accessibility}{mask_emoji} - {zaal_name} ({cinema_name}, {cinema_address}) - \x1B]8;;{ticket_link}\x1B\\Tickets\x1B]8;;\x1B\\")
...
def print_movies_by_title(search_query, movies, schedules, cinemas, screens):
    ...
    if matching_movies:
        ...
        for movie in matching_movies:
            movie_schedules = [schedule for schedule in schedules if schedule['movie_id'] == movie['id']]
            print_movie(movie, movie_schedules, cinemas, screens)
    else:
...
def parse_date(date_str):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Invalid date format. Please use YYYY-MM-DD, 'today' or 'tomorrow'.")
...
def main():
    ...
    parser.add_argument("-m", "--movie", type=str, help="Search for a movie by title. Can be combined with -d.")
